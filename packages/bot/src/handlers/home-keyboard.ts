...
export async function handleRandomBrowse(ctx: Context, botId: number) {
  ...
  const shareInfo = { botId, resourceId: resource.id, caption: resource.caption, inviteLinkId: botUser.inviteLinkId };
  const mediaCounts = {
...
export async function handleFavoriteBrowse(ctx: Context, botId: number) {
  ...
  const shareInfo = { botId, resourceId: first.resource.id, caption: first.resource.caption, inviteLinkId: botUser.inviteLinkId };
  const mediaCounts = {
    total: allMediaFiles.length,
    visible: visibleMediaFiles.length,
    hidden: allMediaFiles.length - visibleMediaFiles.length,
